...
const UserProfileDropdown: React.FC<UserProfileDropdownProps> = ({
  ...
  return (
    <Menu as="div" className="relative inline-block text-left">
      {({ open }) => (
        <>
          <Menu.Button className="group flex items-center space-x-3 px-3 py-2 rounded-xl bg-white/10 backdrop-blur-sm border border-white/20 hover:bg-white/20 hover:border-white/30 transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-purple-500/50">
            <div className="relative">
              {discordAvatar ? (
                <img
                  src={discordAvatar || "/placeholder.svg"}
                  alt="User Avatar"
                  className="h-8 w-8 rounded-full object-cover ring-2 ring-white/20 group-hover:ring-white/40 transition-all duration-300"
                />
              ) : (
                <div className="flex h-8 w-8 items-center justify-center rounded-full bg-gradient-to-br from-purple-500 to-cyan-500 ring-2 ring-white/20 group-hover:ring-white/40 transition-all duration-300">
            ...
            <ChevronDown
              className={cn(
                "h-4 w-4 text-gray-400 group-hover:text-white transition-all duration-300",
                open && "rotate-180",
              )}
            />
          </Menu.Button>
